...
import cn.bingoogolapple.refreshlayout.salemobile.activitypulllist.PullableListViewActivity;
import android.support.v4.app.FragmentManager;
import android.support.v4.app.FragmentPagerAdapter;
import android.support.v4.app.Fragment;
import android.support.v4.app.*;
import cn.bingoogolapple.refreshlayout.salemobile.activitypulllist.*;
/**
...
public class NormalListViewFragment extends BaseFragment implements BGARefreshLayout.BGARefreshLayoutDelegate, AdapterView.OnItemClickListener, AdapterView.OnItemLongClickListener, BGAOnItemChildClickListener, BGAOnItemChildLongClickListener {
   ...
   // private BGAViewPager mViewPager;  //tchl 20160123
...
/*        mMorePageNumber++;
    ...
    public void onItemChildClick(ViewGroup parent, View childView, int position) {
        if (childView.getId() == R.id.tv_item_normal_storage) {
            Log.i(TAG, "tchl:check storage item:"+mAdapter.getItem(position).getBrandNum()+"  "+mAdapter.getItem(position).getBrand());
            //mAdapter.removeItem(position);

/*            Intent intent = new Intent();
            intent.setClass(getActivity(), PullableListViewActivity.class);
            intent.putExtra("index", 1);
            startActivity(intent);*/
            Intent intent = new Intent();
            intent.setClass(getActivity(), StoragePullList.class);
